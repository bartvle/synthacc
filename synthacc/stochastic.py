...
class OmegaSquareSourceModel(Object):
    ...
    def cf(self):
        """
        Corner frequency (in Hz). See Boore (2003) eq. 4.
        """
...
class GeometricalSpreadingModel(Object):
    ...
    def plot(self, distances, max_distance=None, title=None, size=None, png_filespec=None):
        """
        """
...
class QModel(Object):
    ...
    def plot(self, frequencies, min_q=None, max_q=None, title=None, size=None, png_filespec=None):
        """
        """
...
class AttenuationModel(Object):
    ...
    def plot(self, distances, frequencies):
        """
        """
        ...
        # for i, f in enumerate(frequencies):
            ...
            # else:
                # for j in range(len(self._factors)):
                    # if f >= self._factors[:,0][j] and f < self._factors[:,0][j+1]:
                        # slope = np.log10(self._factors[:,1][j+1]/self._factors[:,1][j])/(self._factors[:,0][j+1]-self._factors[:,0][j])
                        # factors[i] =  self._factors[:,1][j]*10**(slope*(f-self._factors[:,0][j]))
                        # found = True
                        # break
                # assert(found)
        # return factors
...
# def plot_geometrical_spreading_models(gsms, distances, labels=None, max_distance=None, title=None, size=None, png_filespec=None):
#     """
#     """
...
# def plot_q_models(q_models, frequencies, min_q=None, max_q=None, title=None, size=None, png_filespec=None):
#     """
#     """
#     fig, ax = plt.subplots(figsize=size)
...
# def plot_attenuation_models(ams, distances, frequencies, title=None, size=None, png_filespec=None, validate=True):
#     """
#     """
#     if validate is True:
#         pass
