"""
The 'io.syngine' module.

Module to get data from the IRIS Synthetics Engine (Syngine).

Syngine (Krischer et al., 2017) is a webservice that quickly returns synthetic
seismograms custom requested by the user. The IRIS DMC stores precalculated TB
scale databases of Green's Functions for several different 1D reference models.
The Green's Functions were generated by Tarje Nissen-Meyer & colleagues using
AxiSEM (Nissen-Meyer et al., 2014), an axi-symmetric spectral element method
for 3D (an-)elastic, anisotropic and acoustic wave propagation in spherical
domains. It requires axisymmetric background models and runs within a 2D
computational domain, thereby reaching all desired highest observable
frequencies (up to 2Hz) in global seismology. The requested synthetic
seismograms are calculated using Instaseis (van Driel et al., 2015), a Python
library built for storing wavefield databases with a fast extraction algorithm.
